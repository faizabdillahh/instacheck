...
// --- Utility: extract usernames from various Instagram JSON structures ---
const extractUsernames = (data, sourceType) => {
    ...
    try {
        ...
        console.error(`Error extracting ${sourceType}:`, err);
        return null;
    }
...
const readFileAsJSON = (file) => new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
        ...
        catch (err) { reject(new Error('Format JSON tidak valid. Pastikan file adalah JSON yang benar.')); }
    };
...
const handleFile = async (file, sourceType) => {
    ...
    if (!file.name.endsWith('.json')) {
        setStatus(zone, statusEl, labelEl, file.name, false, 'Hanya file .json');
    ...
    try {
        ...
        if (!usernames || usernames.size === 0) {
            throw new Error(
                ...
                'Pastikan file benar dari Instagram dan pilih "JSON" saat ekspor. ' +
                'Lihat panduan langkah III untuk detail.'
            );
        }
...
DOM.followingInput.addEventListener('change', (e) => {
    if (e.target.files[0]) handleFile(e.target.files[0], 'following');
});

// Drag & drop
['followers', 'following'].forEach(type => {
    const zone = type === 'followers'
        ? DOM.followersZone.querySelector('.upload-zone')
        : DOM.followingZone.querySelector('.upload-zone');
...
const analyze = () => {
    ...
    const notFollowingBack = [...followingData]
        .filter(u => !followersData.has(u))
        .sort();
    const notFollowedBack = [...followersData]
        .filter(u => !followingData.has(u))
        .sort();
...
const renderTab = (tab) => {
    activeTab = tab;
    const list = tab === 'not-following-back'
        ? currentResults.notFollowingBack
        : currentResults.notFollowedBack;
...
DOM.exportBtn.addEventListener('click', () => {
    const list = activeTab === 'not-following-back'
        ? currentResults.notFollowingBack
        : currentResults.notFollowedBack;
    ...
    const label = activeTab === 'not-following-back'
        ? 'Kamu follow, mereka tidak'
        : 'Mereka follow, kamu tidak';
